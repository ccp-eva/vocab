...
```{r}
# MPI
# rsync -zaP ccp-odcJ:/srv/ccp-odc/clt-extended/data/ ~/Work/Local/Quantex/vocab/data/clt_raw_data/

# ZAS und kischawi

# rsync -zaP ccp-odcJ:/srv/ccp-odc/clt-zas/data/ ~/Work/Local/Quantex/vocab/data/clt_raw_data_zas/
```
...
```{r}
write_csv(data,"../data/clt_clean_data")
```
...
bind_rows(
  ...
  #write_xlsx("../data/clt_reminder.xlsx")
```
...
data%>%
  ...
  ggplot(aes(x = age))+
  geom_jitter(aes(y = 0),width =0, height = 0.02, pch = 1, alpha =.75)+
  geom_density()+
  #ylim(-1,1)+
  theme_minimal()
```
...
```{r}
p1 <- data%>%
  group_by(targetWord, aoa_german_comb)%>%
