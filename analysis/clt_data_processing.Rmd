...
dem <- bind_rows(
  ...
  read_xlsx("../../../../Cloud/Shared/CLT_Kinderlisten_fuer_Emailversandt_14032022.xlsx", sheet = 19),
  )%>%
...
old_part <- read_xlsx("../data/participated_06_04_22.xlsx")%>%
  pull(subjID)

filtered_data%>%
  filter(trial == 1)%>%
  filter(!subjID%in%old_part)%>%
  distinct(subjID, .keep_all = T)%>%
  left_join(dem)%>%
...
data%>%
  filter(age_group != 8, 
         source == "mpi")%>%
  write_csv(.,"../data/clt_clean_data.csv")
```
...
# reminder2 <- read_xlsx("../../../../Cloud/Shared/CLT_Kinderlisten_fuer_Emailversandt_14032022.xlsx", sheet = 16)
# reminder3 <- read_xlsx("../../../../Cloud/Shared/CLT_Kinderlisten_fuer_Emailversandt_14032022.xlsx", sheet = 17)
# reminder4 <- read_xlsx("../../../../Cloud/Shared/CLT_Kinderlisten_fuer_Emailversandt_14032022.xlsx", sheet = 20)
# 
# bind_rows(
#   read_xlsx("../../../../Cloud/Shared/CLT_Kinderlisten_fuer_Emailversandt_14032022.xlsx", sheet = 1),
#   read_xlsx("../../../../Cloud/Shared/CLT_Kinderlisten_fuer_Emailversandt_14032022.xlsx", sheet = 2),
...
#   filter(!code %in% reminder2$code)%>%
#   filter(!code %in% reminder3$code)%>%
#   filter(!code %in% reminder4$code)%>%
#   filter(code != "RY5sK3xF")%>%
#   filter(code != "s6fdepZJ")%>%
#   write_xlsx("../data/clt_reminder5.xlsx")
```
...
```{r}
split <- data%>%
  mutate(test_half = ifelse(trial%%2 == 0, "even","odd"))%>%
  group_by(age,order,subjID, test_half)%>%
  summarise(mean = mean(correct))%>%
  pivot_wider(names_from = test_half, values_from = mean)
  
ggplot(split, aes(x = even, y = odd, col = age))+
  geom_abline(intercept = 0, slope = 1, lty = 3, alpha = .75)+
  geom_jitter(pch= 1, width = 0.01, height = 0.01)+
...
rel <- filtered_data%>%
  filter(subjID %in%rep)%>%
  mutate(correct = ifelse(targetWord == chosenWord, 1,0))%>%
  group_by(subjID)%>%
  mutate(all_trial = 1:length(trial),
         session = ifelse(all_trial > 52, "round2", "round1"))%>%
  left_join(dem)%>%
  select(-timestamp)%>%
  ungroup()%>%
  mutate(age = as.numeric(difftime(test_day, birth_day, "days"))/365.25,
  ...
  filter(!is.na(age))%>%
  ungroup()%>%
  group_by(subjID, age, session)%>%
  summarise(mean = mean(correct))%>%
  pivot_wider(names_from = session, values_from = mean)
  
ggplot(rel, aes(x = round1, y = round2, col = age))+
  geom_abline(intercept = 0, slope = 1, lty = 3, alpha = .75)+
  geom_point(pch= 1)+
  scale_color_viridis_c()+
  stat_cor()+
...
ggplot(sex_dif, aes(x = sex, y = mean))+
...
ggplot(order_dif, aes(x = order, y = mean))+
  geom_bar(stat = "identity", alpha = .25)+
  geom_pointrange(aes(ymin = ci_lower, ymax = ci_upper), alpha = .75)+
  theme_minimal()
```
