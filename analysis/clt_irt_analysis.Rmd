...
irt3_dif_sex <- add_criterion(irt3_dif_sex, "loo") 

saveRDS(irt3_dif_sex, "../saves/irt3_dif_sex.rds")
```

```{r}
loo_compare(irt3, irt3_dif_sex)%>%as_tibble(rownames = "model")
```



```{r}
coef_irt3_dif_sex <- coef(irt3_dif_sex)

eta_sex <- bind_rows(
  coef_irt3_dif_sex$targetWord[, , "eta_sexf"] %>%
	as_tibble(rownames = "item")%>%mutate(sex = "female"),
  coef_irt3_dif_sex$targetWord[, , "eta_sexm"] %>%
	as_tibble(rownames = "item")%>%mutate(sex = "male")
  )%>%mutate(nlpar = "Easiness")

alpha_sex <- 
  bind_rows(
    coef_irt3_dif_sex$targetWord[, , "logalpha_sexf"] %>%
	exp() %>%
	as_tibble(rownames = "item")%>%mutate(sex = "female"),
	    coef_irt3_dif_sex$targetWord[, , "logalpha_sexm"] %>%
	exp() %>%
	as_tibble(rownames = "item")%>%mutate(sex = "male")
...
ggplot(params_sex,aes(reorder(item, -aoa_german_comb), Estimate, ymin = Q2.5, ymax = Q97.5)) +
	facet_wrap("nlpar", scales = "free_x") +
...
irt3_dif_order <- add_criterion(irt3_dif_order, "loo") 

saveRDS(irt3_dif_order, "../saves/irt3_dif_order.rds")
```

```{r}
loo_compare(irt3, irt3_dif_order)%>%as_tibble(rownames = "model")
```

```{r}
coef_irt3_dif_order <- coef(irt3_dif_order)

eta_order <- bind_rows(
  coef_irt3_dif_order$targetWord[, , "eta_orderorderA"] %>%
	as_tibble(rownames = "item")%>%mutate(order = "A"),
  coef_irt3_dif_order$targetWord[, , "eta_orderorderB"] %>%
	as_tibble(rownames = "item")%>%mutate(order = "B")
  )%>%mutate(nlpar = "Easiness")

alpha_order <- 
  bind_rows(
    coef_irt3_dif_order$targetWord[, , "logalpha_orderorderA"] %>%
      exp() %>%
      as_tibble(rownames = "item")%>%mutate(order = "A"),
    coef_irt3_dif_order$targetWord[, , "logalpha_orderorderB"] %>%
      exp() %>%
      as_tibble(rownames = "item")%>%mutate(order = "B")
  )%>%mutate(nlpar = "Discrimination")
...
ggplot(params_order,aes(reorder(item, -aoa_german_comb), Estimate, ymin = Q2.5, ymax = Q97.5)) +
	facet_wrap("nlpar", scales = "free_x") +
