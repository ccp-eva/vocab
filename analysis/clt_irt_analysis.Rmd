...
```{r}
irt1 <- add_criterion(irt1, "loo", moment_match = TRUE) 
irt2 <- add_criterion(irt2, "loo", moment_match = TRUE) 
irt3 <- add_criterion(irt3, "loo", moment_match = TRUE) 
...
remove <- c("eye" ,"telephone" ,"strawberry", "plane" ,"snake" ,"fish", "giraffe", "bicycle" ,"belt")
...
firt2 <- mirt(firt_dat, 1, itemtype = "2PL",  guess = .25)
...
format <- l.format%>%left_join(names)

# plot chart
ggplot(format, aes(Theta, P.1)) + 
  geom_line() + 
...
```
```{r}
plot(firt2, type='trace', which.item = c(1:43), facet_items=T, 
