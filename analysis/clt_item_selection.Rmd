...
sim_a <- simulated_annealing(15)

items <- model_params%>%filter(nlpar=="Easiness")
sel_items <- items$item[sim_a == TRUE]
...
model_params%>%
  ...
  ggplot(aes(reorder(item, -aoa_german_comb), Estimate, ymin = Q2.5, ymax = Q97.5)) +
