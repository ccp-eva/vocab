...
```{r}
ggarrange(page, paoa, ncol = 1)
```
```{r}
ggsave("../graphs/data_fig.png", width = 4, height = 6, scale = 1.25)
```
...
psel <- ggplot()+
  ...
  labs(x = "No. of items in subset", y = "Correlation with full test")
```

```{r}
pparam <- all_model_params%>%
  filter(item %in% frq_sel_items)%>%
  mutate(nlpar = factor(nlpar, levels = c("Easiness", "Discrimination")))%>%
...
```{r}
ggsave("../graphs/item_fig.png", width = 12, height = 5, scale = 1.25, bg = "white")
```
