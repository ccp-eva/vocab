...
```{r}
mc1 <- loo_compare(irt1, irt1_guess, irt2, irt3)%>%as_tibble(rownames = "model")
...
ggplot(dif_sex,aes(reorder(item, -aoa_german_comb), mode, ymin = lci, ymax = uci)) +
	geom_pointrange(aes(col = sex), position = position_dodge(width = .5)) +
	coord_flip() +
  scale_color_colorblind(labels = c("male","female"))+
	labs(x = "Item")+
