...
```{r}
coef_irt3 <- coef(irt3) # robust = T (default = F)
...
```{r}
mc1 <- loo_compare(irt1, irt1_guess, irt3)%>%as_tibble(rownames = "model")
...
```{r}
loo_compare(irt1_guess_sel, irt3_sel, criterion = "waic")%>%as_tibble(rownames = "model")%>%
  left_join(loo_compare(irt1_guess_sel, irt3_sel, criterion = "loo")%>%as_tibble(rownames = "model")%>%select(model, elpd_loo, se_elpd_loo, p_loo, se_p_loo, looic, se_looic))
```
...
pers_params <- ranef(irt1_guess_sel)$subjID%>%as_tibble(rownames = "subjID")
...
# Manually excluding items

```{r}
irt_dat_sel_man <- irt_dat_sel%>%
  filter(!targetWord %in% c("badger", "jaguar","satellite"))

irt1_guess_sel_man <- brm(
  data = irt_dat_sel_man,
  family = brmsfamily("bernoulli", "identity"),
  bf(
    correct ~ 0.25 + 0.75 * inv_logit(eta),
    eta ~ 1 + (1 | targetWord) + (1 | subjID),
...
loo_compare(irt1_guess_sel_man, irt3_sel_man)%>%as_tibble(rownames = "model")


```

# Correlation item difficulty and frequency

```{r}
plot_dif <- coef(irt1_guess)$targetWord %>%
	as_tibble(rownames = "targetWord")%>%
  left_join(
...
plot_dif%>%
  #filter(german != "tür", 
   #      german != "auge")%>%
  ggplot(aes(x = value, y = Estimate.eta_Intercept))+
  geom_point(pch = 1)+
