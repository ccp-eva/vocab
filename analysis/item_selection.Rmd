...
score_fn <- function(subset) {
  ...
  mean_pkd <- mean(pkd[subset])/4
	...
	# 
	# return(spacing + mean_discrim/6 + cor/3)
	return(spacing + mean_infit + mean_outfit+ var_disc_2PL +mean_pkd)
}
...
simulated_annealing_rasch <- function(k, cooling_ratio=0.999, reset_thresh=1000, break_thresh=10000) {
  ...
  easiness <- irt1_guess_itemloo_idxs %>% arrange(targetWord)%>%pull(threshold_mode)
  infit <- irt1_guess_itemloo_idxs %>% arrange(targetWord)%>% pull(infit_mode)
  outfit <- irt1_guess_itemloo_idxs%>% arrange(targetWord) %>% pull(outfit_mode)
  disc_2PL <- coef_irt3$targetWord[, , "logalpha_Intercept"] %>% as_tibble(rownames = "item")%>%pull(Estimate)
