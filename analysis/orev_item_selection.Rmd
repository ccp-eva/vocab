...
sim_rasch_test <- simulated_annealing_rasch(20)

sel_items_rasch_test <- items[unlist(sim_rasch_test$best_subset) == TRUE]

sub_rasch_test <- data%>%
  filter(targetWord %in% sel_items_rasch_test,
         )%>%
...
```{r}
ggsave("../graphs/ICC_1PL_sel.png", height = 6, width = 10, scale = 1)
```
